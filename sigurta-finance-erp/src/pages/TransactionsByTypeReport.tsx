...
import { Home, Tag, LayoutDashboard, Download, Printer, Search } from "lucide-react";
...
export default function TransactionsByTypeReport() {
  ...
  const loadTransactions = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        // تحويل القيود إلى معاملات مفردة
        ...
        // فلترة حسب نوع المستند إذا تم إدخاله
        let filtered = allTransactions;
        if (docType.trim()) {
          filtered = allTransactions.filter(t => 
            t.docType?.toLowerCase().includes(docType.trim().toLowerCase())
          );
        }
  ...
  const exportCSV = () => {
    const header = ["نوع المستند", "رقم القيد", "التاريخ", "رقم المستند", "الوصف", "رمز الحساب", "اسم الحساب", "مدين", "دائن", "الرصيد", "التاجر", "مركز التكلفة"];
    ...
    a.href = url;
    a.download = `transactions_by_type_${fromDate}_${toDate}.csv`;
  ...
  // تجميع حسب نوع المستند
  const groupedByType = transactions.reduce((acc, t) => {
    const type = t.docType || "غير محدد";
    if (!acc[type]) {
      acc[type] = [];
    }
    acc[type].push(t);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl font-bold text-white">تقرير المعاملات حسب النوع</h1>
                <p className="text-sm text-purple-100">عرض جميع المعاملات المحاسبية مرتبة حسب نوع المستند</p>
              </div>
            </div>
          ...
          <CardContent className="pt-6">
            <div className="flex flex-wrap gap-4">
              ...
              <Badge variant="default" className="text-base px-4 py-2">
                عدد الأنواع: {Object.keys(groupedByType).length}
              </Badge>
              <Badge variant="default" className="text-base px-4 py-2">
                    ...
                    {transactions.map((transaction) => (
                      <TableRow key={transaction._id}>
                        <TableCell className="text-right font-medium">
                          <Badge variant="outline">{transaction.docType || "غير محدد"}</Badge>
                        </TableCell>
