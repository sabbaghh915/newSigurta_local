...
const VehicleSchema = new Schema(
  {
    ...
    classification: { type: String, trim: true, default: "0" }, // التصنيف (غير حكومية/حكومية/حسم طابع/إعفاء طابع)
    vehicleCode: { type: String, trim: true }, // النوع (نوع المركبة)
    category: { type: String, trim: true }, // الفئة (خاصة/عامة/حكومي/تأجير)
    governorate: { type: String, trim: true }, // المحافظة

    chassisNumber: { type: String, required: true, trim: true },
    engineNumber: { type: String, trim: true },
    engineCapacity: { type: String, trim: true }, // سعة المحرك
    seatsNumber: { type: Number, min: 1, max: 60 }, // عدد الركاب

    brand: { type: String, required: true, trim: true }, // الصانع (manufacturer)
    model: { type: String, required: true, trim: true }, // الطراز
    ...
    notes: { type: String, trim: true, required: true }, // ✅ ملاحظات أصبحت إجبارية
