...
import { Home, Users, LayoutDashboard, Download, Printer } from "lucide-react";
...
export default function GeneralLedgerByDealerReport() {
  ...
  const loadDealers = async () => {
    try {
      // جلب قائمة التجار من مراكز التكلفة (كبديل مؤقت)
  ...
  const loadLedger = async () => {
    ...
    try {
      ...
      const res = await api.get("/erp/reports/general-ledger-by-dealer", { params });
      if (res.data.success) {
      ...
      console.error("Error loading general ledger by dealer:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const exportCSV = () => {
    ...
    a.href = url;
    a.download = `general_ledger_by_dealer_${accountId}_${dealerId || "all"}_${from || "all"}_${to || "all"}.csv`;
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl font-bold text-gray-800">دفتر الأستاذ حسب التاجر</h1>
                <p className="text-sm text-gray-800">دفتر الأستاذ الشامل حسب التاجر</p>
              </div>
            </div>
      ...
      <div className="container mx-auto p-6 space-y-6">
        ...
        <Card className="shadow-md">
          ...
          <CardContent className="space-y-4">
            ...
            {/* Account & Dealer Info */}
            {(selectedAccount || selectedDealer) && (
              ...
              <div className="overflow-auto" dir="rtl">
                ...
                {!entries.length && accountId && (
                  <div className="text-center text-muted-foreground py-10">
                    لا توجد حركات لهذا الحساب والتاجر في الفترة المحددة
                  </div>
                )}
