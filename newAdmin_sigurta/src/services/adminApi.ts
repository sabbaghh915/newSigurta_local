...
export const adminApi = {
  ...
  // Payments
  getPayments: (params?: { from?: string; to?: string; populate?: boolean }) => {
    const qs = new URLSearchParams();
    if (params?.from) qs.set("from", params.from);
    if (params?.to) qs.set("to", params.to);
    if (params?.populate !== false) qs.set("populate", "1");
    const suffix = qs.toString() ? `?${qs.toString()}` : "?populate=1";
    return apiFetch(`/payments${suffix}`);
  },
