    ...
    const load = async () => {
      try {
        ...
        if (candidatePaymentId) {
          ...
          // ✅ إذا كان insuranceCompany ObjectId، نحتاج لجلب تفاصيل الشركة
          if (payment?.insuranceCompany && typeof payment.insuranceCompany === 'object' && payment.insuranceCompany._id) {
            try {
              const company = await apiFetch(`/api/insurance-companies/${payment.insuranceCompany._id}`);
              if (company) {
                payment.insuranceCompany = company;
              }
            } catch (e) {
              console.warn("Failed to fetch insurance company details:", e);
            }
          }
        ...
        // ✅ التأكد من وجود العنوان - البحث في payment أو vehicleLocal كـ fallback
        const address = 
          vehicle?.address || 
          (payment?.vehicleId && typeof payment.vehicleId === 'object' ? payment.vehicleId.address : null) ||
          vehicleLocal?.address || 
          "";
        ...
        const newPolicy: PolicyData = {
          ...
          address: address, // ✅ استخدام العنوان المحسوب
          ...
          // ✅ شركة التأمين
          insuranceCompany: payment?.insuranceCompany || null,
            ...
            <CardContent className="p-8 space-y-8">
              ...
              {policyData.insuranceCompany && (
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-gray-50 p-6 rounded-lg">
                    <div>
                      <p className="text-sm text-gray-600 mb-1">اسم الشركة</p>
                      <p className="text-lg font-bold text-primary">
                        {typeof policyData.insuranceCompany === 'object' 
                          ? policyData.insuranceCompany.name 
                          : policyData.insuranceCompany}
                      </p>
                    </div>
                    {typeof policyData.insuranceCompany === 'object' && policyData.insuranceCompany.code && (
                      <div>
                        <p className="text-sm text-gray-600 mb-1">رمز الشركة</p>
                        <p className="text-lg font-medium font-mono">{policyData.insuranceCompany.code}</p>
                      </div>
                    )}
                    {typeof policyData.insuranceCompany === 'object' && policyData.insuranceCompany.phone && (
                      <div>
                        <p className="text-sm text-gray-600 mb-1">رقم الهاتف</p>
                        <p className="text-lg font-medium font-mono">{policyData.insuranceCompany.phone}</p>
                      </div>
                    )}
                    {typeof policyData.insuranceCompany === 'object' && policyData.insuranceCompany.address && (
                      <div>
                        <p className="text-sm text-gray-600 mb-1">عنوان الشركة</p>
                        <p className="text-lg font-medium">{policyData.insuranceCompany.address}</p>
                      </div>
                    )}
                  </div>
                </div>
