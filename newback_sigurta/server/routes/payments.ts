...
router.get("/", protect, async (req: AuthRequest, res: Response) => {
  try {
    const { status, search, populate, showMine, from, to } = req.query as any;
  ...
  .filter((p: any) => p?.vehicleId) // يشيل null/undefined
  .map((p: any) => {
    const v = p.vehicleId;
    const isObj = v && typeof v === "object";
    return {
      ...p,
      vehicle: isObj ? v : undefined,
      vehicleId: isObj ? String((v as any)?._id) : String(v),
    };
  });
