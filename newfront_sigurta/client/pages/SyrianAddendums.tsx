...
export default function SyrianAddendums() {
  ...
  const loadAddendums = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/addendums?vehicleType=syrian", {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl font-bold text-gray-800">الملاحق السورية</h1>
                <p className="text-sm text-gray-800">عرض وإدارة ملاحق السيارات السورية</p>
              </div>
            </div>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-primary-600">إضافة ملحق جديد للسيارات السورية</p>
                </div>
                <Button
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-primary">{addendums.length}</p>
                </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              الملاحق السورية ({filteredAddendums.length})
            </CardTitle>
          </CardHeader>
          <CardContent>
            {loading && (
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="bg-primary-100">
                      <TableHead className="text-right font-bold text-primary-900 bg-primary-100 border-r-2 border-primary-300">#</TableHead>
                      <TableHead className="text-right font-bold text-primary-900 bg-primary-100 border-r-2 border-primary-300">رقم الملحق</TableHead>
                      <TableHead className="text-right font-bold text-primary-900 bg-primary-100 border-r-2 border-primary-300">نوع الملحق</TableHead>
                      ...
                      <TableHead className="text-right font-bold text-primary-900 bg-primary-100 border-r-2 border-primary-300">المالك</TableHead>
                      <TableHead className="text-right font-bold text-primary-900 bg-primary-100 border-r-2 border-primary-300">المبلغ</TableHead>
                      <TableHead className="text-right font-bold text-primary-900 bg-primary-100 border-r-2 border-primary-300">تاريخ الإصدار</TableHead>
                  ...
                  <TableBody>
                    {paginatedAddendums.length === 0 ? (
                      ...
                      paginatedAddendums.map((addendum, idx) => {
                        ...
                        return (
                          <TableRow key={addendum._id || idx} className={`${idx % 2 === 0 ? "bg-white" : "bg-primary-50"} hover:bg-primary-100 border-b-2 border-primary-300`}>
                            <TableCell className="text-sm border-r-2 border-primary-300 text-center">{globalIndex}</TableCell>
                            <TableCell className="text-sm border-r-2 border-primary-300 text-right font-mono">{addendum.addendumNumber || "—"}</TableCell>
                            <TableCell className="text-sm border-r-2 border-primary-300 text-right">{getAddendumTypeLabel(addendum.addendumType)}</TableCell>
                            <TableCell className="text-sm border-r-2 border-primary-300 text-right">{vehicle?.plateNumber || "—"}</TableCell>
                            <TableCell className="text-sm border-r-2 border-primary-300 text-right">{vehicle?.ownerName || "—"}</TableCell>
                            <TableCell className="text-sm border-r-2 border-primary-300 text-right">{formatMoney(addendum.amount)}</TableCell>
                            <TableCell className="text-sm border-r-2 border-primary-300 text-right">{formatDate(addendum.issuedAt)}</TableCell>
                            <TableCell className="text-sm border-r-2 border-primary-300 text-center">{getStatusBadge(addendum.status)}</TableCell>
                          </TableRow>
                        );
                      })
                    )}
