...
export default function AdminLayout() {
  ...
  const pages: PageInfo[] = [
    ...
    { path: "/admin/reports", name: "التقارير", icon: <BarChart3 className="w-4 h-4" /> },
  ];
  ...
  const navItemClass = ({ isActive }: any) =>
    cn(
      ...
      isActive
        ? "text-white shadow"
        : "text-slate-700",
      isActive && "bg-gradient-to-r from-purple-300 to-purple-200"
    );
  ...
  return (
    <div className="min-h-screen" dir="rtl">
      ...
      <header className="sticky top-0 z-50">
        <div className="bg-gradient-to-r from-violet-700 to-violet-800 text-white shadow-lg">
          {/* ✅ FULL WIDTH */}
          <div className="w-full px-4 sm:px-6 lg:px-8">
            <div className="h-16 flex items-center justify-between gap-3">
              <div className="flex items-center gap-3 min-w-0">
                <img src={logo} alt="Syrian Insurance Federation" className="w-12 h-12 rounded-lg bg-white/20 p-1" />
                <div className="min-w-0">
                  <div className="font-bold text-lg leading-5 truncate text-white">لوحة الإدارة</div>
              ...
              <div className="flex items-center gap-2">
                <Button
                  ...
                  size="sm"
                  onClick={() => navigate("/")}
                  className="bg-white/95 text-violet-700 hover:bg-white gap-2 border-white/30"
                >
              ...
              <NavLink
                ...
                className={({ isActive }) =>
                  cn(
                    ...
                    isActive
                      ? "bg-gradient-to-r from-purple-300 to-purple-200 text-violet-900 border-transparent shadow"
                      : "bg-white/70 hover:bg-white border-violet-600 text-slate-700"
                  )
                }
              >
                {p.icon}
      ...
      <div className="w-full px-4 sm:px-6 lg:px-8 py-6 grid grid-cols-12 gap-6">
        ...
        <aside className="col-span-12 lg:col-span-2 xl:col-span-2">
          <div className="rounded-2xl bg-white/70 backdrop-blur border-2 border-violet-600 shadow-sm p-3 space-y-1">
            {pages.map((p) => (
        ...
        <main className="col-span-12 lg:col-span-10 xl:col-span-10">
          <div className="rounded-2xl bg-white/70 backdrop-blur border-2 border-violet-600 shadow-sm p-4 sm:p-6">
            <Outlet />
          </div>
        </main>
      </div>
