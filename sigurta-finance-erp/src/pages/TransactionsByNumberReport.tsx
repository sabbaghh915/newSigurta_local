...
import { Home, Hash, LayoutDashboard, Download, Printer, Search } from "lucide-react";
...
export default function TransactionsByNumberReport() {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        allTransactions.sort((a, b) => {
          const numA = parseInt(a.entryNumber || "0");
          const numB = parseInt(b.entryNumber || "0");
          return numB - numA; // ترتيب تنازلي
        });

        // فلترة حسب رقم القيد إذا تم إدخاله
        let filtered = allTransactions;
        if (entryNumber.trim()) {
          filtered = allTransactions.filter(t => 
            t.entryNumber?.includes(entryNumber.trim())
          );
        }
  ...
  const exportCSV = () => {
    const header = ["رقم القيد", "التاريخ", "نوع المستند", "رقم المستند", "الوصف", "رمز الحساب", "اسم الحساب", "مدين", "دائن", "الرصيد", "التاجر", "مركز التكلفة"];
    ...
    a.href = url;
    a.download = `transactions_by_number_${fromDate}_${toDate}.csv`;
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl font-bold text-white">تقرير المعاملات حسب الرقم</h1>
                <p className="text-sm text-purple-100">عرض جميع المعاملات المحاسبية مرتبة حسب رقم القيد</p>
              </div>
            </div>
