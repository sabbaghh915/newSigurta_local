...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (syrianRaw) {
      ...
      setPricingInput({
        ...
        electronicCard: Boolean(data.electronicCard ?? true), // ✅ مفعل تلقائياً
        premiumService: Boolean(data.premiumService ?? false),
        rescueService: Boolean(data.rescueService ?? false),
      });
      
      // ✅ تحديث صلاحية البوليصة: تاريخ صلاحية الرخصة = نهاية التأمين
      // بداية البوليصة = اليوم (أو يمكن اختيار تاريخ قبل 3 أشهر)
      const now = new Date();
      setPolicyStartAt(toDatetimeLocalValue(now));
  ...
  const handlePayment = async () => {
    if (!vehicleData) return; // ✅ إزالة شرط paymentMethod
    ...
    try {
      ...
      // ✅ صلاحية البوليصة تبدأ من نفس اليوم (أو التاريخ الذي وضعه الموظف)
      const startAtObj = parseDatetimeLocal(policyStartAt) || new Date();
      
      // ✅ نهاية التأمين = تاريخ صلاحية الرخصة (من localStorage) - التاريخ المكتوب هو نهاية التأمين
      const licenseExpiryDate = stored?.data?.licenseExpiryDate;
      let endAtObj: Date;
      if (licenseExpiryDate) {
        // تاريخ صلاحية الرخصة = نهاية التأمين
        endAtObj = new Date(licenseExpiryDate);
        // إذا كان التاريخ في الماضي، نضيف المدة
        if (endAtObj < startAtObj) {
          endAtObj = addMonthsSafe(startAtObj, months);
        }
      } else {
        // fallback: حسب المدة
        endAtObj = addMonthsSafe(startAtObj, months);
      ...
      const payRes = await paymentApi.create({
        ...
        policyNumber,
        amount: quote.total,
        paymentMethod: (paymentMethod || "cash") as any, // ✅ افتراضي نقدي
  ...
  // ✅ نهاية التأمين = تاريخ صلاحية الرخصة (من localStorage)
  const stored = getStoredVehicle();
  const licenseExpiryDate = stored?.data?.licenseExpiryDate;
  let endAtObj: Date;
  if (licenseExpiryDate) {
          ...
          {o.label} ({o.value})
            ...
            <Card className="shadow-lg">
              <CardHeader>
                ...
                <CardDescription>صلاحية البوليصة تبدأ من نفس اليوم أو من التاريخ الذي وضعه الموظف. يمكن إصدار بوليصة قبل 3 أشهر حسب الصلاحيات.</CardDescription>
              </CardHeader>

              <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <Label>تاريخ/وقت بداية البوليصة *</Label>
                  <Input 
                    type="datetime-local" 
                    value={policyStartAt} 
                    onChange={(e) => setPolicyStartAt(e.target.value)}
                    min={(() => {
                      const minDate = new Date();
                      minDate.setMonth(minDate.getMonth() - 3); // قبل 3 أشهر
                      return toDatetimeLocalValue(minDate).split('T')[0] + 'T00:00';
                    })()}
                  />
                  <p className="text-xs text-gray-500">يبدأ من نفس اليوم أو قبل 3 أشهر حسب الصلاحيات. نهاية التأمين = تاريخ صلاحية الرخصة المدخل في الخطوة الأولى.</p>
                </div>
